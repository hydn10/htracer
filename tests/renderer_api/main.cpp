...
concept renders_without_samples = requires(Renderer const &renderer, ht_f64::scene const &scene) {
  renderer.render(htracer::rendering::unseq, scene);
};

template<typename Renderer>
concept renders_with_samples = requires(Renderer const &renderer, ht_f64::scene const &scene) {
  renderer.render(htracer::rendering::unseq, scene, htracer::rendering::samples_per_pixel{1});
};
...
main()
{
  ...
  auto const lvalue_image = lvalue_renderer.render(htracer::rendering::unseq, scene);
  ...
  auto const temporary_renderer = htracer::rendering::make_renderer(
      ...
      ht_f64::point_sensor{},
      ht_f64::pinhole_lens{});
  auto const temporary_image = temporary_renderer.render(htracer::rendering::unseq, scene);
  ...
  auto const borrowed_renderer = htracer::rendering::make_renderer(
      ...
      std::cref(borrowed),
      ht_f64::pinhole_lens{});
  auto const borrowed_image = borrowed_renderer.render(htracer::rendering::unseq, scene);
