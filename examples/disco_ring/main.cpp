  ...
  return std::views::iota(0u, size) |
         std::views::transform(
             [=](auto idx)
             {
               ...
               auto const tilted =
                   vec{flat_pos.template get<0>(),
                       -flat_pos.template get<2>() * sin(tilt),
                       flat_pos.template get<2>() * cos(tilt)};

               return vec{
                   tilted.template get<0>() * cos(spin) + tilted.template get<2>() * sin(spin),
                   tilted.template get<1>(),
                   tilted.template get<0>() * -sin(spin) + tilted.template get<2>() * cos(spin)};
             });
}
