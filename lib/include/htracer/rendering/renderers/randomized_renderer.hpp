...
#include <htracer/utils/generator_providers/borrowed_generator_provider.hpp>
...
class randomized_renderer
{
  ...
  [[nodiscard]]
  image<Float>
  render(ExPolicy && /*policy*/, Scene const &scene, samples_per_pixel samples, GeneratorProvider &&rep) const;
    ...
    ExPolicy &&policy, Scene const &scene, samples_per_pixel samples, GeneratorProvider &&rep) const
{
  ...
  samplers::detail_::repeat_sampler const sampler(samples.value(), std::forward<GeneratorProvider>(rep));
    ...
    ExPolicy &&policy, Scene const &scene, samples_per_pixel samples) const
{
  ...
  else
  {
    ...
    utils::generator_providers::detail_::borrowed_generator_provider<random_engine> random_engine_provider(gen);
